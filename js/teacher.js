...
async function init() {
  ...
  if (classInfoEl) {
    const subjectsHtml = `<h4>Subjects You Teach</h4>
      <ul class="pill-list">${me.subjects.map(s => `<li>${s}</li>`).join('')}</ul>`;
    const departments = [...new Set(
      data.students
        .filter(s => s.subjects.some(sub => me.subjects.includes(sub)))
        .map(s => s.department)
    )];
    const departmentsHtml = `<h4>Associated Departments</h4>
      ${departments.length
        ? `<ul class="pill-list">${departments.map(d => `<li>${d}</li>`).join('')}</ul>`
        : `<p class="muted">No departments detected.</p>`}`;
  ...
  studentsYouTeach = data.students.filter(s =>
    s.subjects.some(sub => me.subjects.includes(sub))
  );
...
function renderTeacherTable(filterSubject = '') {
  ...
  studentsYouTeach.forEach(st => {
    const studentScores = data.scores.filter(
      r =>
        r.studentId === st.id &&
  ...
  tbody.querySelectorAll('.submit-remark').forEach(btn => {
    btn.addEventListener('click', e => {
      ...
      const input = tbody.querySelector(
        `.remark-input[data-student="${stId}"][data-assessment="${aId}"]`
      );
      const newRemark = input.value;
      const score = data.scores.find(
        s => s.studentId === stId && s.assessmentId === aId
      );
      if (score) {
        score.remarks = newRemark;
        console.log(`Remark submitted for student ${stId}, assessment ${aId}: ${newRemark}`);
      }
    });
  });
}
  ...
  document.getElementById('exportTeacherCsv').addEventListener('click', () => {
    ...
    studentsYouTeach.forEach(st => {
      const scores = data.scores.filter(
        s => s.studentId === st.id && me.subjects.includes(s.subject)
      );
      scores.forEach(s => {
